...
class KAPMReservoirContract extends KlaytnContract_1.default {
    constructor() {
        super("0x14b72d1fa82d131d81fdaea5a9378b65587a13e1", KAPMReservoir_json_1.default.abi);
    ...
    async sendToken(toChain, receiver, amount) {
        await this.runWalletMethod("sendToken", toChain, receiver, amount, ethers_1.constants.AddressZero);
    }
    async sendedAmounts(sender, toChainId, receiver, sendingId) {
        return bignumber_1.BigNumber.from((await this.runMethod("sendingData", sender, toChainId, receiver, sendingId))[0]);
    }
    async sendingBlock(sender, toChainId, receiver, sendingId) {
        return bignumber_1.BigNumber.from((await this.runMethod("sendingData", sender, toChainId, receiver, sendingId))[1]);
    }
    ...
    async receiveToken(sender, fromChain, receiver, amount, sendingId, isFeePayed, vs, rs, ss) {
        await this.runWalletMethod("receiveToken", sender, fromChain, receiver, amount, sendingId, isFeePayed, ethers_1.constants.AddressZero, vs, rs, ss);
    }
