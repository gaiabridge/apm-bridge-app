        ...
        if (this.fromForm.sender !== undefined &&
            ...
            if (receiver === _receiver) {
                try {
                    ...
                    const senderDiscountRate = Config_1.default.discountUsers.includes(sender) === true ? 5000 : 0;
                    const vs = [];
                    const rs = [];
                    const ss = [];
                    for (const url of Config_1.default.oracleURLs) {
                        ...
                        const result = await superagent_1.default.get(`${url}?${params.toString()}`).send();
